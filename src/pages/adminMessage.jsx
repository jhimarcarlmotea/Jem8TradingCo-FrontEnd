              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                  {Array.isArray(selected.messages) && selected.messages.map((msg) => {
                    const fromMe = msg.from === "me" || msg.sender === "admin" || msg.is_admin;
                    const senderName = msg.sender_name || msg.sender?.name || (fromMe ? "Admin" : selected.name?.split(" ")[0]);
                    return (
                      <div key={msg.id || msg.created_at || Math.random()} className={`flex items-end gap-2 ${fromMe ? "justify-end" : "justify-start"}`}>
                        {!fromMe && (
